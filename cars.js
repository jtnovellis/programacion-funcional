...
function ferraris(cars) {
    let ferrCars = cars.filter(car => car.brand === 'Ferrari');
    return ferrCars;
};


function nineties(cars) {
    let model_80_90 = cars.filter(car => car.model >= 1980 && car.model <= 1990); 
    return model_80_90;
};


function list(cars){
    return cars.map( car => `This ${car.brand} from ${car.model} costs ${car.price}`);
};
